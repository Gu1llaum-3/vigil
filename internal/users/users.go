...
func (um *UserManager) InitializeUserSettings(e *core.RecordEvent) error {
	record := e.Record
	// initialize settings with defaults (zero values can be ignored)
	settings := struct {
...
// Mimics previous default behavior in PocketBase < 0.23.0 allowing user creation through the app UI.
