...
type Config struct {
	...
	// UseMirror specifies whether to use a configured mirror instead of GitHub API.
	// When false (default), always uses api.github.com. When true, uses MirrorHost if configured.
	UseMirror bool
...
func (p *updater) update() (updated bool, err error) {
	...
	if p.config.Owner == "" {
		p.config.Owner = app.ReleaseOwner
	}

	if p.config.Repo == "" {
		p.config.Repo = app.ReleaseRepo
	}

	if p.config.ArchiveExecutable == "" {
		p.config.ArchiveExecutable = app.HubBinary
	}

	if p.config.MirrorHost == "" {
		p.config.MirrorHost = app.ReleaseMirrorHost
	}
	...
	apiURL := getApiURL(p.config.UseMirror, p.config.MirrorHost, p.config.Owner, p.config.Repo)
	if p.config.UseMirror && p.config.MirrorHost != "" {
	...
	releaseDir := filepath.Join(p.config.DataDir, app.UpdateTempDirName)
	...
	if err := downloadFile(p.config.Context, p.config.HttpClient, asset.DownloadUrl, assetPath, p.config.UseMirror, p.config.MirrorHost); err != nil {
		return false, err
	}
...
func FetchLatestRelease(ctx context.Context, client HttpClient, url string) (*release, error) {
	if url == "" {
		url = getApiURL(false, app.ReleaseMirrorHost, app.ReleaseOwner, app.ReleaseRepo)
	}
...
func downloadFile(
	...
	if useMirror && mirrorHost != "" {
		url = strings.Replace(url, "github.com", mirrorHost, 1)
	}
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return err
...
func getApiURL(useMirror bool, mirrorHost, owner, repo string) string {
	if useMirror && mirrorHost != "" {
		return fmt.Sprintf("https://%s/repos/%s/%s/releases/latest?api=true", mirrorHost, owner, repo)
	}
