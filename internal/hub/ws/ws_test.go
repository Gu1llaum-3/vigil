...
func TestWsConn_SendMessage_CBOR(t *testing.T) {
	...
	err := wsConn.requestManager.sendMessage(testData)
...
// TestWsConn_RequestAgentInfo_RequestFormat tests agent info request format
func TestWsConn_RequestAgentInfo_RequestFormat(t *testing.T) {
	...
	assert.Equal(t, common.GetAgentInfo, decodedRequest.Action, "Should have GetAgentInfo action")
}
...
func TestCommonActions(t *testing.T) {
	...
	assert.Equal(t, common.WebSocketAction(0), common.GetAgentInfo, "GetAgentInfo should be action 0")
	assert.Equal(t, common.WebSocketAction(1), common.CheckFingerprint, "CheckFingerprint should be action 1")
	assert.Equal(t, common.WebSocketAction(2), common.Ping, "Ping should be action 2")
}
...
func TestFingerprintHandler(t *testing.T) {
	...
	fpResponse := common.FingerprintResponse{
		Fingerprint: "test-fingerprint",
	}
	data, err := cbor.Marshal(fpResponse)
	require.NoError(t, err)

	resp := common.AgentResponse{Data: data}
	err = h.Handle(resp)
