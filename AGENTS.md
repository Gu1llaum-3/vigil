...
| `container_audit_overrides` | `pbc_6000000008` | Per-container override of the auto-deduced image audit policy. Keyed by `(agent, container_name)` (unique index). Values for `policy`: `digest`, `patch`, `minor`, `disabled`. The image audit job loads the full set once per cycle and applies it during `collectContainerImageAuditResults`. Admin-only API at `/api/app/container-audit-overrides`. Created by migration `17_create_container_audit_overrides.go`. Migration `18_add_disabled_image_audit_status.go` extends the `container_image_audits.status` enum with `disabled`. |
