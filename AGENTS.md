...
| `system_notifications` | `pbc_6000000009` | Append-only authenticated-user event feed for the navbar bell and `/notifications` page. Independent from notification channels/rules/logs. Read cursors and category preferences live in `user_settings.settings`. Created by migration `19_create_system_notifications.go`. |
