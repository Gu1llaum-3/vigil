...
- If the task is “mute/silence notifications for a resource, or change notification suppression”, read `docs/backend/hub-backend.md` (Notification Dispatcher → Suppression chokepoint), `internal/hub/notification_mute.go`, and the `notification_mutes` collection in `docs/architecture/auth-and-data-model.md`; the frontend lives in `internal/site/src/lib/mutes.ts` + `components/mute-menu.tsx`.
- If the task is “add or change maintenance windows or the maintenance banner”, read `docs/backend/hub-backend.md` (Notification Dispatcher → Suppression chokepoint), `internal/hub/maintenance.go` + `maintenance_api.go`, and the `maintenance` collection in `docs/architecture/auth-and-data-model.md`; the frontend is `internal/site/src/components/maintenance-banner.tsx` + `components/routes/settings/maintenance.tsx`.
...
│   │   ├── host_metrics.go         # host metrics polling, persistence, hosts overview/detail APIs
│   │   ├── container_metrics.go    # container metrics persistence, retention, host detail history API
...
| `agents` | `pbc_4000000001` | One record per agent. Key fields: `token`, `fingerprint`, `status`, `capabilities`, `metadata`, `tags`. The `token` field is **hidden** (migration `27_hide_agent_token.go`) so it is not exposed via the generic collection API/realtime; non-readonly users fetch tokens via `GET /api/app/agent-tokens` and rotate via `POST /api/app/agents/{id}/rotate-token`. `tags` (JSON array of free-text strings, migration `32_add_agent_tags.go`) groups/searches/filters hosts; it is a normal writable field — non-readonly users edit it directly via the collection API from the hosts table, host detail page, or Settings → Agents. All three render tags through the shared `components/host-tags.tsx` (`<HostTags>`: a compact fixed-height "cloud" with a `+N` overflow tooltip in the dense hosts table, wrapped elsewhere) and edit through `components/tags-dialog.tsx`. Tags are surfaced in the hosts-overview/detail payloads. Agents may also declare initial tags via the `TAGS` env var, applied by the hub **only at first enrollment** (`setInitialAgentTags`), never overwriting later UI edits. |
...
| `monitors` | `pbc_5000000002` | One record per uptime monitor. Type: `http`/`ping`/`tcp`/`dns`/`push`. Status written by scheduler via `SaveNoValidate`. Created by migration `3_create_monitors.go`. Migration `8_add_monitor_inverted.go` adds `inverted` (bool — flip up↔down so a reachable target is the alert condition). Migration `9_add_monitor_ip_family.go` adds `ip_family` (select `""`/`ipv4`/`ipv6`, `""` = auto/dual-stack) — pins **HTTP and TCP** checks to one IP family via the dial network (`tcp4`/`tcp6`), e.g. to dodge a flaky IPv6 path to a Cloudflare-proxied target; empty keeps Go's default Happy Eyeballs. **The single-digit `8_`/`9_` prefixes (not `32_`/`35_`) are deliberate — PocketBase applies migrations in lexical filename order, so a two-digit prefix would sort before `3_create_monitors.go` and the field-add would silently skip; see `docs/conventions-and-gotchas.md`.** |
| `monitor_events` | `pbc_5000000003` | Append-only check history. Relation to `monitors` (cascadeDelete=true). Indexed on `(monitor, checked_at)`. Created by migration `3_create_monitors.go`. `status` per event is `1`=up, `0`=down, or **`2`=pending** (a failed check still under the monitor's `failure_threshold` — the monitor's own `status` is never `2`). Pending drives the amber sparkline bars and is **excluded from the uptime denominator** (`up/(up+down)`); written by `saveResult` in `internal/hub/monitors.go`. A `maintenance` bool (migration `9_add_monitor_event_maintenance.go` — the `9_` prefix sorts **after** `3_create_monitors.go` so the ALTER lands after the collection exists; see `docs/conventions-and-gotchas.md`) marks checks taken inside an active maintenance window covering the monitor (set from the in-memory `maintenanceWindowCache` → no DB query per check); such rows are excluded from the uptime aggregates entirely. |
...
| `system_notifications` | `pbc_6000000009` | Append-only authenticated-user event feed for the navbar bell and `/notifications` page. Independent from notification channels/rules/logs. Read cursors and bell event preferences live in `user_settings.settings`. Created by migration `19_create_system_notifications.go`. |
...
| `container_image_audits` | `pbc_6000000006` | Latest read-only image audit result per `(agent, container_id)` for Docker images discovered in snapshots. Any Registry v2 endpoint is supported (Docker Hub, GHCR, Quay, GitLab, self-hosted Harbor, etc.). Authentication is resolved via multi-keychain: `registry_credentials` collection first, then host `~/.docker/config.json`, then anonymous. Stores normalized image metadata, audit policy, status, latest candidate tag/digest, and last check error/timestamp. Migration `30_add_image_audit_failure_tracking.go` adds `consecutive_failures`/`last_check_error`/`last_check_error_at`: a transient registry failure keeps the last good status (escalating to `check_failed` only after 3 consecutive failures), so a one-off timeout doesn't flash red. Registry calls are serialized per host + per-call timeout (`image_audits_runner.go`) to avoid self-inflicted rate-limiting. Created by migration `13_create_container_image_audits.go`. |
| `registry_credentials` | `pbc_6000000007` | Per-registry basic-auth credentials used by the image audit job for private registries. Admin-only (collection rules are `null`; access is gated by the `requireAdminRole` middleware on `/api/app/registry-credentials`). Passwords encrypted at rest with AES-256-GCM using `<datadir>/credentials.key`. API redacts the password as `**REDACTED**`. Unique index on `registry`. Created by migration `16_create_registry_credentials.go`. |
| `container_audit_overrides` | `pbc_6000000008` | Per-container override of the auto-deduced image audit policy. Keyed by `(agent, container_name)` (unique index). Values for `policy`: `digest`, `patch`, `minor`, `disabled`. The image audit job loads the full set once per cycle and applies it during `collectContainerImageAuditResults`. Optional `tag_include` / `tag_exclude` Go regexes narrow the candidate tag list before semver comparison (priority include > variant > exclude); the current tag is always preserved so the baseline never becomes unreachable. Admin-only API at `/api/app/container-audit-overrides`. Created by migration `17_create_container_audit_overrides.go`. Migration `18_add_disabled_image_audit_status.go` extends the `container_image_audits.status` enum with `disabled`. Migration `20_add_tag_filters_to_container_audit_overrides.go` adds the `tag_include` / `tag_exclude` text fields. |
| `user_api_keys` | `pbc_8000000001` | Long-lived bearer tokens for non-browser clients (scripts, the MCP server) to authenticate to `/api/app/*` as the owning user. Only the **SHA-256 hash** of the token is stored (`token_hash`, hidden); the plaintext (`vk_…`) is returned **once** at creation. Fields: `name`, `created_by` (relation→users, cascadeDelete), `prefix` (display head), `scope` (`read`/`read-write`), `last_used_at`, `expires_at`. Collection rules are `null` — access is only through the per-user `/api/app/api-keys` handlers and the `authenticateApiKey` middleware (`internal/hub/api_keys.go`). Keys authenticate **only the Vigil app API** (`/api/app/*` and `/api/mcp`), never the generic PocketBase API (`/api/collections`, `/api/realtime`, admin). A `read` key may only use safe HTTP methods; on `/api/mcp` scope is enforced per-tool by serving the read-only tool set (`mcpHandler`). `expires_at` is validated (RFC3339, future) at creation; key management is refused when authenticated via a key (no self-minting). Created by migration `31_create_user_api_keys.go`. |
| `notification_mutes` | `pbc_9000000001` | Per-resource notification silence list. Suppresses **both** the in-app bell and external channels for one resource, checked at the `h.emitNotification` chokepoint via `isNotificationSuppressed` (`internal/hub/notification_mute.go`). Fields: `resource_type` (`monitor`/`agent`/`container_image`), `resource_id` (**plain text, no FK**), `muted_until` (date, empty = indefinite), `created_by`, `note`. Unique index on `(resource_type, resource_id)` → muting is an upsert. For `monitor`/`agent` the id is the record id. For `container_image` the id is `<agentID>\|<containerName>` (the stable container **name**, matching `container_audit_overrides`) — *not* the ephemeral container id the event carries, so a mute survives the redeploy it was meant to silence; the hub resolves the event's container id to its name (from the event Details) when checking suppression. A mute on an `agent` also covers that host's `container_image` events. No FK means an orphan row after the resource is deleted is harmless (no event references it). Rules: read = authenticated, write = non-readonly (frontend uses `pb.collection` directly). Created by migration `33_create_notification_mutes.go`. |
| `maintenance` | `pbc_9000000002` | Planned maintenance windows. While a window is active it (a) suppresses notifications for the covered resources at the `h.emitNotification` chokepoint (via `underMaintenance`/`maintenanceCoversEvent` in `internal/hub/maintenance.go`) and (b) drives the global "maintenance in progress" banner. `strategy` is `single` (absolute `start_at`/`end_at`) or `recurring` (local `start_time`/`end_time` in `timezone`, optional `weekdays` JSON `[0..6]` and `active_from`/`active_to` date bounds; handles midnight-crossing). `scope` (JSON, same shape as `notification_rules.filter`) is `{}` = global or `{monitor_ids,agent_ids}` = targeted; a `container_image` event is covered by its parent agent. `severity` (`info`/`warning`/`critical`) colors the banner. Writes are admin-only: create/update/delete rules are `null` and CRUD is gated by `requireAdminRole` on `/api/app/maintenance-windows` (handlers use `app.Save`, bypassing rules). **Read** (list/view) is open to any authenticated user so the banner can subscribe over realtime and react instantly to a teammate's change; the banner still renders from the slim authenticated `GET /api/app/maintenance/active` and uses the collection only as a change signal. Active-window computation is lazy (no cron); the banner also polls (60s) for recurring windows that activate by wall-clock and emit no DB event. v1 suppresses **all** events in-window (the Kuma "MAINTENANCE→DOWN still alerts" nuance is a future refinement). Created by migration `34_create_maintenance.go`. |
...
- **API keys** (`Authorization: Bearer vk_…`) — long-lived tokens for scripts / the MCP server, scoped read or read-write, managed at `/api/app/api-keys` and in Settings → API keys. See the `user_api_keys` collection and `authenticateApiKey` in `internal/hub/api_keys.go`.
