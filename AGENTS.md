...
| `monitors` | `pbc_5000000002` | One record per uptime monitor. Type: `http`/`ping`/`tcp`/`dns`/`push`. Status written by scheduler via `SaveNoValidate`. Created by migration `3_create_monitors.go`. Migration `8_add_monitor_inverted.go` adds `inverted` (bool — flip up↔down so a reachable target is the alert condition). **The `8_` prefix (not `32_`) is deliberate — PocketBase applies migrations in lexical filename order, so `32_` would sort before `3_create_monitors.go`; see `docs/conventions-and-gotchas.md`.** |
