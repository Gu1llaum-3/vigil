...
func TestNewWebSocketClient(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.token != "" {
				t.Setenv(app.AgentEnvPrefix+"TOKEN", tc.token)
			}
...
func TestWebSocketClient_GetOptions(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/api/app/agent-connect",
		},
		{
		...
		},
		{
			...
			expectedScheme: "ws",
			expectedPath:   "/custom/path/api/app/agent-connect",
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// Set up environment
			t.Setenv(app.AgentEnvPrefix+"HUB_URL", tc.inputURL)
			...
			assert.Equal(t, app.Version, options.RequestHeader.Get("X-App"))
...
func TestWebSocketClient_HandleHubRequest(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "GetAgentInfo without verification",
...
func TestGetToken(t *testing.T) {
	...
	t.Run("token from prefixed TOKEN environment variable", func(t *testing.T) {
		// Set the prefixed TOKEN env var (should take precedence)
		expectedToken := "test-token-from-app-env"
		t.Setenv(app.AgentEnvPrefix+"TOKEN", expectedToken)
	...
	t.Run("token from prefixed TOKEN_FILE", func(t *testing.T) {
		// Create a temporary token file
		expectedToken := "test-token-from-app-file"
		tokenFile, err := os.CreateTemp("", "token-test-*.txt")
		require.NoError(t, err)
		defer os.Remove(tokenFile.Name())
		...
		// Set the prefixed TOKEN_FILE env var (should take precedence)
		t.Setenv(app.AgentEnvPrefix+"TOKEN_FILE", tokenFile.Name())
	...
	t.Run("error when neither TOKEN nor TOKEN_FILE is set", func(t *testing.T) {
		...
		t.Setenv("TOKEN", "")
		t.Setenv(app.AgentEnvPrefix+"TOKEN_FILE", "")
		t.Setenv("TOKEN_FILE", "")
