...
func GetDataDir(dataDirs ...string) (string, error) {
	...
	if runtime.GOOS == "windows" {
		dataDirs = append(dataDirs,
			filepath.Join(os.Getenv("APPDATA"), app.AgentDataDirName),
			filepath.Join(os.Getenv("LOCALAPPDATA"), app.AgentDataDirName),
		)
	} else {
		dataDirs = append(dataDirs, filepath.Join("/var/lib", app.AgentDataDirName))
		if homeDir, err := os.UserHomeDir(); err == nil {
			dataDirs = append(dataDirs, filepath.Join(homeDir, ".config", app.AgentConfigDirName))
		}
	}
	return testDataDirs(dataDirs)
}
