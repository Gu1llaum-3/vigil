...
# Function to upgrade vigil-agent with Scoop
function Upgrade-AppAgentWithScoop {
    Write-Host "Upgrading vigil-agent with Scoop..."
    scoop update vigil-agent
    
    if (-not (Test-CommandExists "vigil-agent")) {
        throw "Failed to upgrade vigil-agent with Scoop"
    }
    
    return $(Join-Path -Path $(scoop prefix vigil-agent) -ChildPath "vigil-agent.exe")
}

# Function to upgrade vigil-agent with WinGet
function Upgrade-AppAgentWithWinGet {
    Write-Host "Upgrading vigil-agent with WinGet..."
    ...
    & winget upgrade --exact --id Gu1llaum-3.vigil-agent --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    if (-not $agentPath) {
        ...
        $agentPath = Find-AppAgent
        if (-not $agentPath) {
            throw "Could not find vigil-agent executable path after upgrade"
        }
    }
...
function Get-ServiceConfiguration {
    ...
    if (-not $existingService) {
        throw "vigil-agent service does not exist. Please run the installation script first."
    }
    ...
    try {
        ...
        $envVars = & $nssmCommand get vigil-agent AppEnvironmentExtra
...
function Update-ServicePath {
    ...
    Write-Host "Updating vigil-agent service path..."
    ...
    & $nssmCommand set vigil-agent Application $NewAgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to update vigil-agent service path"
    }
    
    Write-Host "Service path updated to: $NewAgentPath"
    
    # Start the service
    Start-AppAgentService -NSSMPath $nssmCommand
}
...
try {
    Write-Host "App Agent Upgrade Script" -ForegroundColor Cyan
    Write-Host "===========================" -ForegroundColor Cyan
    
    # First: Check if service exists (doesn't require admin)
    $existingService = Get-Service -Name "vigil-agent" -ErrorAction SilentlyContinue
    if (-not $existingService) {
        Write-Host "ERROR: vigil-agent service does not exist." -ForegroundColor Red
    ...
    Write-Host "Stopping vigil-agent service..."
    & $nssmPath stop vigil-agent
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Upgrading vigil-agent..."
    $newAgentPath = $null
    
    if (Test-CommandExists "scoop") {
        Write-Host "Using Scoop for upgrade..."
        $newAgentPath = Upgrade-AppAgentWithScoop
    }
    elseif (Test-CommandExists "winget") {
        Write-Host "Using WinGet for upgrade..."
        $newAgentPath = Upgrade-AppAgentWithWinGet
    }
    else {
    ...
    if (-not $newAgentPath) {
        $newAgentPath = Find-AppAgent
        if (-not $newAgentPath) {
            throw "Could not find vigil-agent executable after upgrade."
        }
    }
    ...
    if ($isAdmin -or $Elevated) {
        ...
        Write-Host ""
        Write-Host "Upgrade completed successfully!" -ForegroundColor Green
        Write-Host "The vigil-agent service has been updated to use the new executable path." -ForegroundColor Green
