...
# Function to find vigil-agent in common installation locations
function Find-AppAgent {
    # First check if it's in PATH
    $agentCmd = Get-Command "vigil-agent" -ErrorAction SilentlyContinue
    if ($agentCmd) {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\scoop\apps\vigil-agent\current\vigil-agent.exe",
        "$env:ProgramData\scoop\apps\vigil-agent\current\vigil-agent.exe",
        "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\Gu1llaum-3.vigil-agent*\vigil-agent.exe",
        "$env:ProgramFiles\WinGet\Packages\Gu1llaum-3.vigil-agent*\vigil-agent.exe",
        "${env:ProgramFiles(x86)}\WinGet\Packages\Gu1llaum-3.vigil-agent*\vigil-agent.exe",
        "$env:ProgramFiles\vigil-agent\vigil-agent.exe",
        "$env:ProgramFiles(x86)\vigil-agent\vigil-agent.exe",
        "$env:SystemDrive\Users\*\scoop\apps\vigil-agent\current\vigil-agent.exe"
    )
...
function Install-Scoop {
    ...
    if (Test-Admin) {
        throw "Scoop cannot be installed with administrator privileges. Please run this script as a regular user first to install Scoop and vigil-agent, then run as admin to configure the service."
    }
...
# Function to install vigil-agent with Scoop
function Install-AppAgentWithScoop {
    Write-Host "Adding app bucket..."
    scoop bucket add app https://github.com/Gu1llaum-3/vigil-scoops | Out-Null
    
    Write-Host "Installing / updating vigil-agent..."
    scoop install vigil-agent | Out-Null
    
    if (-not (Test-CommandExists "vigil-agent")) {
        throw "Failed to install vigil-agent"
    }
    
    return $(Join-Path -Path $(scoop prefix vigil-agent) -ChildPath "vigil-agent.exe")
}

# Function to install vigil-agent with WinGet
function Install-AppAgentWithWinGet {
    Write-Host "Installing / updating vigil-agent..."
    ...
    & winget install --exact --id Gu1llaum-3.vigil-agent --accept-source-agreements --accept-package-agreements | Out-Null
    ...
    # Refresh PATH environment variable to make vigil-agent available in current session
    $env:Path = [System.Environment]::GetEnvironmentVariable("Path", "Machine") + ";" + [System.Environment]::GetEnvironmentVariable("Path", "User")
    
    # Find the path to the vigil-agent executable
    $agentPath = (Get-Command vigil-agent -ErrorAction SilentlyContinue).Source
    
    if (-not $agentPath) {
        throw "Could not find vigil-agent executable path after installation"
    }
...
function Install-WithScoop {
    ...
    try {
        ...
        # Install vigil-agent
        $agentPath = Install-AppAgentWithScoop
...
function Install-WithWinGet {
    ...
    try {
        ...
        # Install vigil-agent
        $agentPath = Install-AppAgentWithWinGet
...
function Install-NSSMService {
    ...
    Write-Host "Installing vigil-agent service..."
    ...
    $existingService = Get-Service -Name "vigil-agent" -ErrorAction SilentlyContinue
    if ($existingService) {
        Write-Host "Service already exists. Checking if path update is needed..."
        
        # Get current service path 
        try {
            $currentPath = & $nssmCommand get vigil-agent Application
        ...
        try {
            & $nssmCommand stop vigil-agent
            & $nssmCommand remove vigil-agent confirm
        } catch {
            Write-Host "Warning: Failed to remove existing service: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
    
    & $nssmCommand install vigil-agent $AgentPath
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to install vigil-agent service"
    }
    
    Write-Host "Configuring service environment variables..."
    & $nssmCommand set vigil-agent AppEnvironmentExtra "+KEY=$Key"
    & $nssmCommand set vigil-agent AppEnvironmentExtra "+TOKEN=$Token"
    & $nssmCommand set vigil-agent AppEnvironmentExtra "+HUB_URL=$HubUrl"
    & $nssmCommand set vigil-agent AppEnvironmentExtra "+PORT=$Port"
    
    # Configure log files
    $logDir = "$env:ProgramData\vigil-agent\logs"
    if (-not (Test-Path $logDir)) {
        New-Item -ItemType Directory -Path $logDir -Force | Out-Null
    }
    $logFile = "$logDir\vigil-agent.log"
    & $nssmCommand set vigil-agent AppStdout $logFile
    & $nssmCommand set vigil-agent AppStderr $logFile
}
...
function Configure-Firewall {
    ...
    Write-Host "Creating firewall rule for vigil-agent on port $Port..."
    try {
...
function Start-AppAgentService {
    param (
        [string]$NSSMPath = ""
    )
    
    Write-Host "Starting vigil-agent service..."
    ...
    if ($startResult -ne 0) {
        ...
        while (-not $serviceStarted -and $elapsedTime -lt $maxWaitTime) {
            ...
            $serviceStatus = & $nssmCommand status vigil-agent
            
            if ($serviceStatus -eq "SERVICE_RUNNING") {
                $serviceStarted = $true
                Write-Host "Success! The vigil-agent service is now running." -ForegroundColor Green
            }
        ...
        if (-not $serviceStarted) {
            ...
            Write-Host "You can check status manually with 'nssm status vigil-agent'" -ForegroundColor Yellow
        }
    } else {
        # NSSM start command was successful
        Write-Host "Success! The vigil-agent service is running properly." -ForegroundColor Green
    }
}
...
try {
    ...
    if (-not $AgentPath) {
        throw "Could not find vigil-agent executable. Make sure it was properly installed."
    }
    ...
    if (-not $isAdmin -and -not $Elevated) {
        ...
        Write-Host "Check service status with 'nssm status vigil-agent'"
        Write-Host "Edit service configuration with 'nssm edit vigil-agent'"
    ...
    if ($isAdmin -or $Elevated) {
        ...
        # Start the service
        Start-AppAgentService -NSSMPath $NSSMPath
