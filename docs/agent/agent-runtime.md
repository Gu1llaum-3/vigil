...
- `metrics.go` — lightweight host monitoring metrics (CPU, memory, root disk, network throughput, 1/5/15-min load average, and the max used% across all real mounted filesystems). The load and max-disk fields back the host metric-threshold alerts; they are append-only CBOR fields, so older hubs ignore them and the hub degrades gracefully for older agents that omit them. To keep this high-frequency path cheap, the mounted-filesystem list is cached for 5 minutes (same interval as network-interface re-detection), network/remote filesystems (NFS/CIFS/…) are excluded so a stalled remote mount can't block collection, and root usage is read once via that same partition loop (`DiskMaxMount` reports the busiest mount).
