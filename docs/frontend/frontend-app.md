...
- `MonitorRow` — per-monitor row: status badge, name + last message + an `Inverted` badge for inverted monitors, type badge, target (clickable for HTTP(S) monitors; push URLs keep their copy button), latency, rolling 24h average latency, 24h uptime, 30d uptime, mini status bars for the last checks, age, action dropdown with a `Move to` submenu. The mini status bars (`checkBarClass`) color each recent check green=up / **amber=pending** (`status===2`, a failed check still under the failure threshold) / red=down, Uptime-Kuma style; pending also shows an amber badge in the detail page's `statusBadge`
...
- `MonitorDetailPage` — dedicated monitor page with larger summary cards, a 1h/3h/6h/24h/**7d** range selector, a latency chart with red down bands, **amber pending bands**, and **blue maintenance bands**, a clickable target URL for HTTP(S) monitors, and a `Mode: Inverted` metadata cell for inverted monitors. Bands are drawn by the shared `useBandPlugin` hook (`components/metric-charts.tsx`, wrapping `createBandPlugin` behind a live-bands ref so bands stay correct across range changes — react-chartjs-2 applies the `plugins` prop only at mount): down/pending come from `buildBands` over the series' status (0 / 2); maintenance comes from `GET /api/app/monitors/{id}/maintenance?range=` via `lib/maintenance.ts` (`fetchMonitorMaintenance` + `occurrenceBands`), fetched alongside the chart/transitions and drawn **underneath** (blue is `MAINTENANCE_BAND_COLOR`; maintenance bands are live-computed from the windows so they render even for retroactively-created windows). The chart fetches raw events for ≤24h; for **7d** it fetches the server-downsampled `GET /api/app/monitors/{id}/series?range=7d` (so a week renders without ~10k points). Both the chart (≤24h events) and the transitions list are windowed server-side via `range` (not a client-computed `since`), so they cover exactly the same period regardless of clock skew. Below the chart, a **Status history** card lists up↔down transitions over the selected range (newest first), fetched via `GET …/events?range=<key>&transitions_only=true&limit=500` — an Uptime-Kuma-style incident log reusing the shared `statusBadge`; pending checks are excluded from this list (they only show on the sparkline), so it stays a real outage log; if the 500-change cap is hit the card notes that older changes are not listed.
- `MonitorDialog` — create/edit form with type-conditional fields plus the failure threshold setting (`0` = instant down, default `3`); reachability monitors (http/tcp/dns/ping, not push) also expose an `Inverted` switch. **HTTP and TCP** monitors expose an `IP family` selector (`Auto`/`IPv4`/`IPv6` → `ip_family`) via the shared `IPFamilyField` component (also used by the ping form), letting a target be pinned to one family (e.g. force IPv4 on a Cloudflare-proxied site to survive an IPv6-path blip)
...
Shared chart components live in `internal/site/src/components/metric-charts.tsx` (`MetricCard`, `MetricBar`, `MetricHistoryChart`, `NetworkHistoryChart`, `LoadHistoryChart`, `FleetMetricChart`, `buildTimeSeries`, `metricsRanges`, plus the shared `createBandPlugin` / `useBandPlugin`) and shared format helpers in `internal/site/src/lib/format.ts`. Both are used by `host-detail.tsx` and `container-detail.tsx`. `MetricHistoryChart`/`NetworkHistoryChart`/`LoadHistoryChart` accept an optional `plugins` prop forwarded to their `<Line>`; `host-detail.tsx` uses it to draw **blue maintenance bands** on every host chart (CPU/memory/disk/network/load) — it fetches `GET /api/app/hosts/{id}/maintenance?range=` (`fetchHostMaintenance` in `lib/maintenance.ts`) alongside the metrics and passes one shared `createBandPlugin` instance to all charts. Hosts have no uptime `%`, so maintenance for hosts is a chart annotation only (notifications are already suppressed in-window at the `emitNotification` chokepoint).

The `/metrics` route (`routes/fleet-metrics.tsx`, sidebar "Metrics") is a fleet-wide view: it plots **all** metrics (CPU/memory/disk/load) across **all** hosts as stacked multi-line charts (one `FleetMetricChart` per metric, a series per host), with only a time-range selector — every chart shows at once so the page can be skimmed without toggling a metric. It fetches everything in one call, `GET /api/app/fleet-metrics?range=` (a metric-keyed map), and refreshes on `host_metric_current` realtime events (1s debounce).
