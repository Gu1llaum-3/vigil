...
- `MonitorDetailPage` — dedicated monitor page with larger summary cards, a 1h/3h/6h/24h/**7d** range selector, a latency chart with red down bands, a clickable target URL for HTTP(S) monitors, and a `Mode: Inverted` metadata cell for inverted monitors. The chart fetches raw events for ≤24h; for **7d** it fetches the server-downsampled `GET /api/app/monitors/{id}/series?range=7d` (so a week renders without ~10k points). Both the chart (≤24h events) and the transitions list are windowed server-side via `range` (not a client-computed `since`), so they cover exactly the same period regardless of clock skew. Below the chart, a **Status history** card lists up↔down transitions over the selected range (newest first), fetched via `GET …/events?range=<key>&transitions_only=true&limit=500` — an Uptime-Kuma-style incident log reusing the shared `statusBadge`; if the 500-change cap is hit the card notes that older changes are not listed.
