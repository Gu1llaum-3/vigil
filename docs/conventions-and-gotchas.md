...
- **loadavg is normalized to load-per-core.** A raw load average is meaningless across hosts of different sizes (load 4 = saturated on 1 core, idle on 10), so the evaluator divides `Load1` by the agent's CPU core count and compares against a **per-core** threshold (`1.0` = fully utilized). The core count comes from the agent's snapshot (`Resources.CPUCores`), cached per agent (refreshed on snapshot upsert, lazy-loaded on miss). If the core count is unknown (no snapshot yet) the loadavg metric is **skipped**, not alerted on a raw value. Migration `28_reset_loadavg_alerts.go` cleared pre-existing rows because their stored values were absolute, not per-core.
