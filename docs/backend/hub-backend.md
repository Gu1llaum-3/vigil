...
- `GET /api/app/hosts/:id/maintenance?range=` — maintenance intervals (`{start,end,title,severity}`) covering this host (agent) over the range, for the host detail charts' blue bands. Reuses `maintenanceOccurrencesForResource("agent", …)`; scope-filtered (global windows or `agent_ids` containing the host). Available to all authenticated users.
...
The rolling uptime and average latency values are computed over the available events in each window. Uptime is `up / (up + down)` — only `status IN (0,1)` events with `maintenance = 0` count toward the denominator, so **pending (`2`) and unknown (`-1`) events, and any check flagged `maintenance`, are excluded** (Uptime-Kuma model: retries under the failure threshold and planned maintenance don't penalize uptime). The `maintenance` flag is set at check time by `saveResult`: it reads the in-memory `maintenanceWindowCache` (`monitorUnderMaintenance`, zero DB queries on the hot path; the cache holds the enabled window *definitions*, kept fresh via `maintenance`-collection after-write hooks plus a 60s safety re-warm ticker, warmed **before** the monitor scheduler starts so a restart mid-window flags correctly, with activeness evaluated live against `now`) and marks the event when an active window covers the monitor. Because the flag is written at check time it is **not retroactive** — a window created after the fact does not re-exclude past events from the `%` (the chart's blue bands, computed live, still cover it). They are exposed as soon as at least one up/down event exists in the window (`total > 0`), so the frontend shows real data immediately. `N/A` is only shown when there are no countable events at all for the window. `avg_latency_24h_ms` is never set for `push`-type monitors.
...
- `GET /api/app/monitors/{id}/maintenance?range=` — maintenance intervals (`{start,end,title,severity}`) covering this monitor over the range, for the detail chart's blue bands. `maintenanceOccurrencesForResource("monitor", …)` enumerates each enabled window's concrete occurrences over `[now-range, now]` via the pure `maintenanceOccurrences(spec, since, until)` helper (single = absolute interval; recurring = per-day enumeration honoring weekday/date bounds and midnight crossing), scope-filtered (global or `monitor_ids` containing the monitor). Available to all authenticated users.
